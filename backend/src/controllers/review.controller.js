 ...
 * @route   GET /api/reviews?partnerId=xyz
 * @access  Public
 */
const getPartnerReviews = asyncHandler(async (req, res) => {
  const { partnerId } = req.query;
  const result = await ReviewService.getPartnerReviews(partnerId);
...
const getMyReviews = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      ApiResponse.success(result, "Your reviews retrieved successfully")
    );
});
...
export { createReview, getPartnerReviews, getMyReviews, deleteReview };
