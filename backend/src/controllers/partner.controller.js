...
// /**
//  * @desc    Partner onboarding
//  * @route   POST /api/partner/onboard
//  * @access  Private (Partner only)
//  */
const onboardPartner = asyncHandler(async (req, res) => {
  const partner = await PartnerService.onboardPartner(req.user._id, req.body);
...
const getPartnerProfile = asyncHandler(async (req, res) => {
  const partner = await PartnerService.getPartnerProfile(req.user._id);

  return res
    .status(200)
    .json(
      ApiResponse.success(partner, "Partner profile retrieved successfully")
    );
});
...
const updatePartnerProfile = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      ApiResponse.success(partner, "Partner profile updated successfully")
    );
});
...
// /**
//  * @desc    Get matched leads
//  * @route   GET /api/partner/leads
//  * @access  Private (Partner only)
//  */
const getMatchedLeads = asyncHandler(async (req, res) => {
  const { status } = req.query;
  const result = await PartnerService.getMatchedLeads(req.user._id, status);

  return res
    .status(200)
    .json(ApiResponse.success(result, "Leads retrieved successfully"));
});
...
const getLeadDetails = asyncHandler(async (req, res) => {
  const lead = await PartnerService.getLeadDetails(req.user._id, req.params.id);

  return res
    .status(200)
    .json(ApiResponse.success(lead, "Lead details retrieved successfully"));
});

// /**
//  * @desc    Respond to lead
//  * @route   PUT /api/partner/leads/:id/respond
//  * @access  Private (Partner only)
//  */
const respondToLead = asyncHandler(async (req, res) => {
  const { responseMessage } = req.body;
  const inquiry = await PartnerService.respondToLead(
    req.user._id,
    req.params.id,
    responseMessage
  );
  ...
  return res
    .status(200)
    .json(ApiResponse.success(inquiry, "Response submitted successfully"));
});
...
const addPortfolioItem = asyncHandler(async (req, res) => {
  const portfolio = await PortfolioService.addPortfolioItem(
    req.user._id,
    req.body
  );
  ...
  return res
    .status(201)
    .json(ApiResponse.created(portfolio, "Portfolio item added successfully"));
});

// /**
//  * @desc    Get partner portfolio
//  * @route   GET /api/partner/portfolio
//  * @access  Private (Partner only)
//  */
const getPartnerPortfolio = asyncHandler(async (req, res) => {
  const result = await PortfolioService.getPartnerPortfolio(req.user._id);

  return res
    .status(200)
    .json(ApiResponse.success(result, "Portfolio retrieved successfully"));
});
...
const updatePortfolioItem = asyncHandler(async (req, res) => {
  const portfolio = await PortfolioService.updatePortfolioItem(
    req.user._id,
    req.params.id,
    req.body
  );
  ...
  return res
    .status(200)
    .json(
      ApiResponse.success(portfolio, "Portfolio item updated successfully")
    );
});
...
const deletePortfolioItem = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(ApiResponse.success({}, "Portfolio item deleted successfully"));
});
